    ...
    if (!node.num) {
        if (is_global) {
            auto gv = GlobalVariable::create(
                node.id, module.get(), var_type, /*is_const=*/false,
        ...
        auto arr_type = ArrayType::get(var_type, num);

        if (is_global) {
            auto gv = GlobalVariable::create(
                node.id, module.get(), arr_type, false,
                ConstantZero::get(arr_type, module.get()));
            scope.push(node.id, gv);
            return gv;
        } else {
            auto addr = builder->create_alloca(arr_type);
            scope.push(node.id, addr);
            return addr;
        }
    }
...
Value *CminusfBuilder::visit(ASTIterationStmt &node) {
    ...
    builder->create_cond_br(cond_i1, bodyBB, exitBB);
